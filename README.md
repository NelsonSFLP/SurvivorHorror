...
2. Abra o terminal **MSYS2 UCRT64** e certifique-se de estar no diretório do projeto (`/c/Users/...`).
...
* **Implementação:** O motor utiliza da pilha de matrizes do OpenGL (`glPushMatrix` e `glPopMatrix`). Ao renderizar o baú de madeira, usa-se matrizes em hierarquia: transla-se para a dobradiça, e aplica `glRotatef` baseado na interação do jogador, e depois translada de volta antes de desenhar a tampa. Isso é reproduzido também no modelo da arma em primeira pessoa, composto por 16 partes geométricas distintas amarradas a um eixo central.
* *Demonstração Visual:*
...
A construção estética da aplicação foi rigorosamente contida dentro das fronteiras da biblioteca legada OpenGL 2.1, utilizando truques de renderização baseados em *Eye Space*, geometria procedural, e o pipeline de Função Fixa.
...
**6. Curvas Bézier Matemáticas (Avaliadores de Hardware)**
...
2. **Exploit de Singuralidade Física e Dano Instântaneo:** Os monstros perseguiam a mesma coordenada global e se fundiam perfeitamente (0.0f de desvio), resultando em ataques combinados na taxa de 200 de dano, causando *instant-kill*. Desenvolvi um algoritmo matemático de repulsão (*Flocking Behavior*) forçando-os a assumir distâncias interpessoais e atacar circundando o jogador.
